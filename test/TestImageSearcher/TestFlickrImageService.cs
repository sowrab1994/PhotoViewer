...
namespace TestImageSearcher
{
    internal class TestFlickrImageService
    {
        ...
        Mock<IHttpRequestService> httpRequestService;
	...
	<photos page=""2"" pages=""1389"" perpage=""100"" total=""138828"">
        ...
        public void SetUp() 
        {
            httpRequestService = new Mock<IHttpRequestService>();
            searcher = new FlickerImageService(httpRequestService.Object);
        }

        [TestCase("Invalid Response")]
        [TestCase("")]
        public void TestInvalidResponseForImageRequest(string response)
        {
            var searchStr = "Sample";
            httpRequestService.Setup(p => p.GetHttpResponse(It.IsAny<string>()))
                .Returns(Task.FromResult(response));
            var searchResponse = searcher.GetImagesForSearchString(searchStr);
            Assert.IsNotNull(searchResponse);
            Assert.AreEqual(0, searchResponse.ResponsePages);
            Assert.IsTrue(searchResponse.IsRequestSuccessful);
            Assert.AreEqual(new ArrayList(), searchResponse.ImagesArray);
        }

        [TestCase("Sample1")]
        [TestCase("Sample2")]
        public void TestValidResponseForImageRequest(string searchStr)
        {
			var urlPrefix = "https://www.flickr.com/services/rest/?method=flickr.photos.search&api_key=c098ab4dc93e9a203a007ad613d1c414&text=";
            httpRequestService.Setup(p => p.GetHttpResponse(urlPrefix + searchStr))
                .Returns(Task<string>.FromResult(sampleXml));
            var searchResponse = searcher.GetImagesForSearchString(searchStr);
            Assert.IsNotNull(searchResponse);
            Assert.AreEqual(1389, searchResponse.ResponsePages);
            Assert.IsTrue(searchResponse.IsRequestSuccessful);
            Assert.AreEqual("https://live.staticflickr.com/65535/52563028656_b4989029eb.jpg", searchResponse.ImagesArray[0]);
        }


		[Test]
		public void TestHTTPRequestReturnsWithException()
		{
			var searchStr = "Sample";
			...
			Assert.IsNotNull(searchResponse);
			Assert.AreEqual(0, searchResponse.ResponsePages);
			Assert.IsFalse(searchResponse.IsRequestSuccessful);
			Assert.AreEqual(new ArrayList(), searchResponse.ImagesArray);
		}

   //     [Test]
   //     public void TestRequestURLWhenPageIsSet()
   //     {
			//var searchStr = "test";
   //         var urlPrefix = "https://www.flickr.com/services/rest/?method=flickr.photos.search&api_key=c098ab4dc93e9a203a007ad613d1c414&text=";
			...
			//	searcher.SetPage(i);
			//	int j = i + 1;
   //             httpRequestService.Setup(p => p.GetHttpResponse(It.<string>("page" + j.ToString())))
   //             .Returns(Task<string>.FromResult(sampleXml));
   //             var searchResponse = searcher.GetImagesForSearchString(searchStr);
   //         }
